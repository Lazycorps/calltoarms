...
export default class extends Vue {
  ...
  get alreadyFriend(): boolean {
    return UserModule.utilisateur.friendships.some((f) => f.friend_id == this.notification.sender_id);
  }
  ...
  private sendYesResponse() {
    ...
    notif.content = this.responseMessage ? this.responseMessage : "I'm in comrade !";
    ...
    this.$gtag.event('AcceptCall', {
      event_category: 'CallToArms',
      event_label: 'accept',
      value: UserModule.username
    });
  }
  private sendNoResponse() {
    ...
    notif.content = this.responseMessage ? this.responseMessage : 'Nope sorry dude';
    notif.game_id = this.notification.game_id;
    notif.user_ids = [this.notification.sender_id];
    this.dialog = false;
    NotificationApi.sendNotification(notif).then(() => {
