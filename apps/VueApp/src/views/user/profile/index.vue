...
export default class extends Vue {
  ...
  public audiosNotificationSelected = '';
  ...
  get fullPathDownload() {
    return `${process.env.BASE_URL}/audio/notifications/${this.audiosNotificationSelected}`;
  }

  mounted() {
    this.audiosNotification.push({ libelle: 'No sound', path: '' });
    this.audiosNotification.push({ libelle: 'Horn', path: 'horn.mp3' });
    this.audiosNotification.push({
      libelle: 'Air raid',
      path: 'air-siren.mp3'
    });
  ...
  private async deleteGame(game: Game) {
    try {
      const test = await GamesLibraryApi.deleteGame(game.id);
      console.log(test);
  ...
  private testAudio() {
    if (this.audiosNotificationSelected) {
      const audio = new Audio(`${process.env.BASE_URL}/audio/notifications/${this.audiosNotificationSelected}`);
      audio.play();
    }
  }
